...
export const App: React.FC = () => {
  ...
  const [areaFilter, setAreaFilter] = useState<'ALL' | 'ON_DUTY' | 'EAST' | 'WEST' | 'FAVORITES'>('ALL');
    ...
    if (areaFilter === 'FAVORITES') {
      ...
      list = list.filter(g => {
        ...
        if (selectedDate) {
          ...
          return d && (d.primaryArea === '東區' || d.innings.some(inn => inn.location.includes('東')));
        }
        return duties.some(d => d.primaryArea === '東區' || d.innings.some(inn => inn.location.includes('東')));
      });
      ...
      list = list.filter(g => {
        ...
        if (selectedDate) {
          const d = duties.find(item => item.date === selectedDate);
          return d && (d.primaryArea === '西區' || d.innings.some(inn => inn.location.includes('西')));
        }
        return duties.some(d => d.primaryArea === '西區' || d.innings.some(inn => inn.location.includes('西')));
      });
    }
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-rkg-crimson via-rkg-crimson-light to-rkg-pink p-6 sm:p-8 text-white mb-6 shadow-elevated">
          <div className="relative z-10 max-w-xl">
            ...
            <h2 className="text-2xl sm:text-3xl font-black tracking-tight mb-2">
              Rakuten Girls 樂天女孩班表
            </h2>
            <p className="text-xs sm:text-sm text-pink-100/90 leading-relaxed font-normal">
              隨時掌握女孩每場賽事的 1-3 局、中場表演與 7-8 局應援站位（一壘東區／三壘西區）。直接點擊卡片可查看個別出勤月曆與 Instagram！
            </p>
          </div>
