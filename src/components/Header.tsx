...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 bg-[#160105]/95 backdrop-blur-md gpu-layer border-b border-amber-500/30 shadow-lg shadow-black/40 transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-2 sm:py-2.5 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-shrink">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-nowrap">
              <h1 className="font-extrabold text-sm sm:text-base md:text-lg text-amber-100 tracking-tight whitespace-nowrap flex items-center gap-1.5">
                <span>{t.appTitle}</span>
                <Sparkles className="w-3.5 h-3.5 text-amber-400 fill-amber-400 animate-pulse hidden sm:inline" />
              </h1>
              <span className="text-[10px] font-black px-2 py-0.2 rounded-full bg-amber-500/20 text-amber-300 border border-amber-400/50 whitespace-nowrap flex-shrink-0 shadow-sm">
                2026 辣酷甜
              </span>
            </div>
            <p className="text-[11px] text-amber-200/70 font-medium truncate hidden md:block">
              {t.appSubtitle}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          ...
          <div className="flex items-center bg-[#130104] p-0.5 sm:p-1 rounded-xl sm:rounded-2xl border border-amber-500/30 shadow-inner">
            <button
          ...
          <button
            ...
            className="hidden md:inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold bg-[#24040b] hover:bg-[#380611] text-amber-200 border border-amber-500/30 shadow-sm transition active:scale-95 whitespace-nowrap"
          >
      ...
      <div className="bg-[#120104]/90 border-t border-amber-500/20 px-3 sm:px-4 py-1 text-[11px] text-amber-200/80">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 sm:gap-2 truncate">
            {isLive ? (
              <span className="inline-flex items-center gap-1 text-emerald-400 font-semibold whitespace-nowrap flex-shrink-0">
                <ShieldCheck className="w-3 h-3 text-emerald-400" />
                <span className="hidden xs:inline sm:inline">Google 試算表即時連線</span>
                <span className="xs:hidden sm:hidden">即時連線</span>
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-amber-300 font-semibold whitespace-nowrap flex-shrink-0">
                <AlertCircle className="w-3 h-3 text-amber-400" />
                <span>離線模式</span>
              </span>
            )}
            <span className="text-amber-500/40">|</span>
            <span className="truncate text-amber-200/70">更新時間：{lastUpdated}</span>
          </div>

          <button
            onClick={onOpenStadiumGuide}
            className="md:hidden text-amber-300 hover:text-amber-200 font-bold hover:underline flex items-center gap-0.5 whitespace-nowrap flex-shrink-0 text-[11px]"
          >
