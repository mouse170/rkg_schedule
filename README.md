...
3. **Lovable 現代極簡設計架構（Modern Clean & OLED Dark Mode）**：
   - 全面導入 Lovable / shadcn/ui 設計哲學與 CSS 變數設計 Token（`clsx` + `tailwind-merge`），淺色採純淨極簡 Slate 灰白搭酒紅，深色採沉浸式深黑鋅色（Zinc / OLED）搭配磨砂毛玻璃。
   - 頂部吸頂毛玻璃控制艙（Glassmorphic Command Bar）與 Bento Pro 微邊框卡片，懸浮微浮起動效與觸感愛心微動效。
4. **Google 試算表多頁籤即時同步**：
...
9. **辣酷甜主題日盛典（9/19、9/20）與專屬看台應援**：
   - **全勤盛典**：全體 27 位女孩全員出席，專區女孩於比賽期間固定在指定專屬看台席位（東下 I～M、西下 I～K、東上 B～D、西上 B～D）全程應援。
   - **主舞台合體演出**：第 5 局下全體女孩於內野主舞台合體演出年度單曲「辣酷甜」。
   - **Stitch 輕量化球場位置圖彈窗**：點擊橫幅開啟球場看台位置圖與應援規則導覽，去除多餘卡片區塊，載入飛快。
   - **專區單一金色站位膠囊**：專區女孩卡片醒目標記單一「全場專區 ‧ [看台代碼]」金色膠囊，姓名下方附看台代碼。
   - **一般看台女孩支援**：未在專區之女孩歸入「一般看台應援女孩」分組，試算表填入前標註「站位待公布（東／西／東R／西R／大樂）」，並在試算表更新後自動即時連動。
   - **座位視角精準對應**：在「我的座位視角」模式中，一壘東區自動涵蓋東下/東上專區女孩，三壘西區自動涵蓋西下/西上專區女孩，並以金色聚焦外框精準標明專區位置。
10. **GitHub Actions 自動化 CI/CD**：
